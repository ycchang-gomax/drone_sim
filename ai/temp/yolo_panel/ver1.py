...
# - Start / Pause / Stop
# - Show Boxes / Show Conf / Auto-lock / Vehicles-only / Swap R-B (color toggle)
# - Click a box to lock (locked box = wider RED + red cross)
# - UDP send {"dx": px, "dy": px, "dz_rate": mps} at send_hz
# - Proper letterbox mapping (scx/scy + offsets) so boxes line up perfectly
# - Shows device (cuda/cpu) and FPS (EMA) on the panel and overlays
# - Confidence number drawn INSIDE the box (bottom-left)
...
class YoloDeepSortPanel(tk.Frame):
    """
    source, backend, width, height, fps: VideoCapture hints
    model_path: yolov8n.pt / yolov8s.pt ...
    classes_keep: e.g. (2,5,7) for car/bus/truck
    host, port, send_hz: UDP target and rate
    dz_rate: constant climb rate (m/s)
    bias_y:  forward bias applied to dy (pixels)
    ema:     smoothing for dx/dy
    """
    def __init__(self, parent,
        ...
        # thresholds / dynamics
        self.conf_th = tk.DoubleVar(value=0.25)
        ...
        # NEW: jitter filter instance (tuned for straight-road stability)
        self._jitter = _JitterFilter(
            win=5,              # 5-frame median
            dead_enter_px=14,   # enter deadband
            dead_exit_px=8,     # exit deadband (hysteresis)
            alpha=self.ema_a,   # reuse your EMA knob
            max_step_px=40      # limit per-frame change
        )
    ...
    def start(self):
        ...
        try:
            ...
            self._running = True
            print(f"[AI] device={self.device}  model={self.model_path}")  # console info
            self.lbl_status.config(text=f"Running ({self.device})")
    ...
    def _process_frame(self, frame_bgr):
        ...
        # --- Build display boxes (prefer YOLO dets), attach conf by IoU ---
        boxes = []                 # list of (x1,y1,x2,y2, tid) used for drawing & clicking
        conf_by_tid = {}
        cx0, cy0 = W/2.0, H/2.0
        ...
        for tr in tracks:
            ...
            for (dx1, dy1, dx2, dy2, dc, dk) in det_cache:
                ...
                if iou > best_iou:
                    best_iou, best_det = iou, (dx1, dy1, dx2, dy2, dc)

            # choose which to display
            if self.tight_boxes.get() and best_det and best_iou >= 0.2:
                x1, y1, x2, y2, det_conf = best_det
        ...
        # keep conf for TIDs across frames
        ...
        if self.lock_id is not None:
            for tid, box, cx, cy, *_ in candidates:
                if tid == self.lock_id:
                    ...
                    if self.sending.get():
                        self._udp.send(self.dx_s, self.dy_s, self.dz_rate)
                    break
        if not had_lock:
    ...
    def _redraw(self):
        ...
        if Image is not None and ImageTk is not None:
            if self.swap_rb.get():
                ...
                rgb = disp.copy()  # (will look wrong if source is BGR)
        ...
        if self.show_boxes.get():
            draw_set = self._boxes
            if self.lock_id is not None:
                # If we have a lock, try to draw ONLY that box…
                locked_only = [b for b in self._boxes if b[4] == self.lock_id]
                if locked_only:
            ...
            for (x1,y1,x2,y2, tid) in draw_set:
                ...
                # slightly widen locked box (6% width, 3% height)
                if locked:
                    w = X2 - X1; h = Y2 - Y1
